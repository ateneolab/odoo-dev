...
class email_trigger(models.Model):
    ...
    def create_send_mail(self, user):
        template_id = self.env.ref(
            "birthday_mail_schedular.email_template_customer_form"
        )
        ...
        if template_id:
            ...
            for server in mail_server_id:
                if not message_id:
                    try:
                        message_id = template_id.send_mail(
                            user.id, force_send=True, server_id=server.id or False
                        )
...
class Company(models.Model):
    ...
    image_birth_date = fields.Binary(
        string="Tarjeta", help="Tarjeta de cumpleaños, limited to 1024x1024px"
    )
