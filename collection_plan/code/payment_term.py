...
class PaymentTerm(models.Model):
    ...
    internal_state = fields.Selection(
        [("created", "Created"), ("invoiced", "Invoiced"), ("receipt", "Receipt")],
        default="created",
    )
    ...
    discount_type = fields.Selection(
        [("fixed_amount", "Monto Fijo"), ("percentage", "Porcentage")],
    ...
    literal = fields.Text(string=_(u"Literal"), compute="_convert_amount_to_literal")
    ...
    def _compute_company(self):
        ...
        if self.plan_id:
            if self.plan_id.contract_id:
                ...
                company_id = (
                    self.plan_id.collection_plan_id.contract_id.campus_id.company_id
                )
    ...
    def do_payment(self):
        _logger.info(
            "do_payment"
        )  # raise wizard for payment, then link it to collection plan, contract
    ...
    def generate_voucher_receipt(self, state, partner_id, company_id, type):
        ...
        journal = self.env["account.journal"].search(
            [
                ...
                (
                    ...
                    "=",
                    self.payment_mode_id.journal_id.default_debit_account_id.code,
                ),
                (
                    "default_credit_account_id.code",
                    "=",
                    self.payment_mode_id.journal_id.default_credit_account_id.code,
                ),
            ]
        )
        ...
        account_receivable_id = self.env["account.account"].search(
            [("code", "=", account_receivable.code), ("company_id", "=", company_id)]
        )
    ...
    def generate_voucher(
        ...
        voucher_id = self.env["account.voucher"].create(voucher_data)
        _logger.info(
            "COMPANY: %s, invoice move company: %s"
            % (company_id, invoice.move_id.line_id[0].company_id)
        )
    ...
    def do_billing(self):
        ...
        invoice_id = self._context.get("default_invoice_id", None)
        default_id = self._context.get("default_id", None)
        ...
        return {
            ...
            "nodestroy": True,
            "target": "new",
            "context": {"payment_id": default_id, "default_invoice_id": invoice_id},
        }

    @api.one
    def confirm(self):
        contract_id = (
            self.plan_id.contract_id or self.plan_id.collection_plan_id.contract_id
        )
        if not contract_id:
            raise except_orm(
                "Error", u"El término de pago no tiene un contrato asociado."
            )
