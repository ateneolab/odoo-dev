...
class sale_order(models.Model):
    ...
    def action_button_confirm(self, cr, uid, ids, context=None):
        ...
        if res and sale_o and sale_o.generate_contract:
            contract_id = self.generate_education_contract(
                cr,
                uid,
                ids,
                context=context
            ...
            if contract_id:
                self.pool.get('sale.order').write(
                    ...
                    uid,
                    ids,
                    {'education_contract_id': contract_id}
                )
                self.pool.get('education_contract.contract').write(
                    cr,
                    uid,
                    [contract_id],
                    {'campus_id': sale_o.operating_unit_id.id},
                    context=context
                )
        return res

    def generate_education_contract(self, cr, uid, ids, context=None):
        contract_id = self.pool.get('education_contract.contract').create(
            cr,
            uid,
            {'sale_order_id': ids},
            context=context
