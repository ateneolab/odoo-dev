...
class Contract(models.Model):
    ...
    verification_id = fields.Many2one(
        "education_contract.verification", string=_(u"Verificación")
    )
    ...
    def enroll(self):
        ...
        for prog in program_ids:
            ...
            try:
                ...
                raise except_orm(
                    "Error",
                    u"Contacte con el administrador para solucionar el siguiente error: (%s)"
                    % e,
                )
    ...
    def to_assigned(self):
        ...
        if not filled:
            raise ValidationError(
                "Debe completar todos los datos del contrato para cambiar a estado 'Asignado'."
            )
        else:
            try:
...
class Beneficiary(models.Model):
    ...
    verification_id = fields.Many2one(
        "education_contract.verification", string=_("Contract verification")
    )
...
class PaymentTerm(models.Model):
    ...
    is_taxes = fields.Boolean(_(u"Muestra impuestos"))
...
class Plan(models.Model):
    ...
    @api.one
    @api.onchange(
        "type", "amount_pay", "qty_dues", "registration_fee", "payment_term_ids"
    )
    @api.depends(
        "type", "amount_pay", "qty_dues", "registration_fee", "payment_term_ids"
    )
        ...
        if self.type:
            if self.type == "funded":
                ...
                if not self.contract_id:
                    if self.qty_dues:
                        amount_monthly = round(
                            (self.amount_pay - payed) / self.qty_dues, 4
                        )
                    else:
                    ...
                    if self.qty_dues:
                        amount_monthly = round(
                            (self.amount_pay - self.registration_fee) / self.qty_dues, 4
                        )
                    else:
                        amount_monthly = round(
                            (self.amount_pay - self.registration_fee), 4
                        )
