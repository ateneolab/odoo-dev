...
class AccountInvoiceTax(models.Model):
    ...
    def compute(self, invoice):
        ...
        for line in invoice.invoice_line:
            if not invoice.discount_scope == 'per_line_before_tax':
                taxes = line.invoice_line_tax_id.compute_all(
                    (line.price_unit * (1 - (line.discount or 0.0) / 100.0)),
                    line.quantity, line.product_id, invoice.partner_id)['taxes']
            else:
                if invoice.discount_type == 'amount':
                    discount_factor = line.discount_amount / line.quantity
                    price = line.price_unit - discount_factor
                    taxes = \
                    line.invoice_line_tax_id.compute_all(price, line.quantity, line.product_id, invoice.partner_id)[
                        'taxes']
                else:
                    taxes = line.invoice_line_tax_id.compute_all(
                        (line.price_unit * (1 - (line.discount or 0.0) / 100.0)),
    ...
    #
    #         taxes = line.invoice_line_tax_id.compute_all(
    #             price, line.quantity, line.product_id, invoice.partner_id)['taxes']
    #
    #         for tax in taxes:
    #             val = {
    #                 'invoice_id': invoice.id,
