    ...
    def create_tax_lines(self, inv):
        ...
        if inv.type in ['out_invoice']:
            ...
            for line in inv_lines:
                ...
                if not taxes_ids or not len(taxes_ids):
                    taxes_ids = product_id.category_id.tax_retention_ids
                    ...
                    if tax['tax_group'] in ['ret_ir', 'ret_vat_b', 'ret_vat_srv']:
                        ...
                        if tax['tax_group'] in ['ret_vat_b', 'ret_vat_srv']:
                            ...
                            if vat_obj and len(vat_obj):
                                base = base * float(vat_obj[0].percent) / 100
...
class account_invoice_tax(models.Model):
    ...
    tmp_account_tax_id = fields.Many2one('account.tax')
    
    @api.model
    def create(self, vals):
        
        account_tax_id = vals.get('tmp_account_tax_id', False)
        
        if account_tax_id:
            account_tax_obj = self.env['account.tax'].search([('id', '=', account_tax_id)])
            
            if account_tax_obj:
                tax_data = {
                    'name': account_tax_obj.name,
                    'account_id': account_tax_obj.account_collected_id.id,
                    'tax_group': account_tax_obj.tax_group,
                    'percent': account_tax_obj.porcentaje,
                    'base_code_id': account_tax_obj.base_code_id.id,
                    'tax_code_id': account_tax_obj.tax_code_id.id,
                }
                
                vals.update(tax_data)
